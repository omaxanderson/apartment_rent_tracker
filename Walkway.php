<?php
	...
	class Walkway extends Apartment {
		...
		function parseData() {
			...
			for ($i = $startIdx; true ; $i++) {
				...
				if (!($tracker['{'] - $tracker['}'])) {
					...
					if (!$data) {
						echo "Error parsing JSON string" . PHP_EOL;
						return false;
					}
		...
		function createFloorplan($floorplan) {
			...
			echo "inserting into floorplan" . PHP_EOL;
			$params = array(
				'floorplan_name' => $floorplan['name'],
			...
			try {
				...
				if (!$insertResult) {
					echo "insert new floorplan failure" . PHP_EOL;
					exit();
				}
			} catch (Exception $e) {
				echo "Error occurred during floorplan insert" . PHP_EOL;
				exit();
			}
			return true;
		}
